...
async function generateTTS(text, outputPath, voice = 'en-US-AriaNeural', rate = '+0%') {
...
function splitIntoChunks(text, maxChars = 10000) {
...
app.get('/api/pdf/:pdfId/chunks', (req, res) => {
    const pdf = pdfStore.get(req.params.pdfId);
    if (!pdf)
        return res.status(404).json({ error: 'PDF not found' });
    res.json({
...
app.get('/api/pdf/:pdfId/chunk/:chunkIndex', (req, res) => {
    const pdf = pdfStore.get(req.params.pdfId);
    if (!pdf)
        return res.status(404).json({ error: 'PDF not found' });
...
app.get('/api/tts/progress/:pdfId/:chunkIndex', (req, res) => {
